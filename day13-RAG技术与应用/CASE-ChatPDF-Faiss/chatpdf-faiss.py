...
# 处理文本并创建知识库，同时保存到磁盘（使用绝对路径）
knowledgeBase = process_text_with_splitter(text, char_page_mapping, save_path=VECTOR_DB_DIR)
print(f"\n向量库文件已保存到: {VECTOR_DB_DIR}")
print(f"  - index.faiss: {os.path.getsize(os.path.join(VECTOR_DB_DIR, 'index.faiss'))} bytes")
print(f"  - index.pkl: {os.path.getsize(os.path.join(VECTOR_DB_DIR, 'index.pkl'))} bytes")
print(f"  - page_info.pkl: {os.path.getsize(os.path.join(VECTOR_DB_DIR, 'page_info.pkl'))} bytes")

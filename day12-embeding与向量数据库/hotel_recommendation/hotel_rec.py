...
df = pd.read_csv(os.path.join(os.path.dirname(__file__), 'Seattle_Hotels.csv'), encoding="latin-1")
# 数据探索
print(df.head())
...
def get_top_n_words(corpus, n=3, k=None):
...
plt.tight_layout()
chart_path = os.path.join(os.path.dirname(__file__), 'hotel_top20_words.png')
plt.savefig(chart_path, dpi=150, bbox_inches='tight')
plt.close()
print(f'Top20 单词柱状图已保存: {chart_path}')
